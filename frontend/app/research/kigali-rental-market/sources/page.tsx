  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-14">
      ...
      <ul className="space-y-4">
        {(data?.sources || []).map((s) => (
          <li key={s.source} className="border rounded-xl p-4 bg-white dark:bg-navy-800">
            <div className="font-medium text-navy-800 dark:text-white">{s.source}</div>
            <div className="text-sm text-gray-500 mt-1">
              Kind: {s.kind}
              {s.observation_count != null ? ` · ${s.observation_count} observations` : ""}
            </div>
          </li>
        ))}
      </ul>
