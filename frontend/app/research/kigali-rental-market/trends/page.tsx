...
export const metadata: Metadata = {
  ...
  description: "Median asking rent over time and observed listing activity for Kigali — verified and external data shown separately.",
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-14 space-y-10">
      ...
      <ResearchChart
        ...
        subtitle={`${data?.verified_label || "KigaliRent Verified"} · USD/month`}
        kind="line"
      ...
      <ResearchChart
        ...
        subtitle={`${data?.external_label || "External Market Observations"} · not confirmed vacancies`}
        kind="line"
        points={(data?.external_median_series || [])
          .filter((p) => p.median_usd != null)
          .map((p) => ({
            label: p.period_end.slice(0, 7),
            value: p.median_usd || 0,
