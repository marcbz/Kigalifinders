  ...
  return (
    <article className="py-20 px-6">
      <div className="max-w-3xl mx-auto">
        <Link href="/blog" className="text-gold-500 text-sm mb-6 inline-block hover:underline">← Back to Blog</Link>
        <div className="text-xs text-gold-500 tracking-widest mb-4">
          {post.category_name?.toUpperCase()} · {post.read_time_minutes} MIN READ
        </div>
        <h1 className="font-serif text-4xl md:text-5xl font-bold text-navy-800 dark:text-white mb-8">{post.title}</h1>
        {post.featured_image && (
        ...
        {post.tags?.length > 0 && (
          <div className="flex flex-wrap gap-2 mt-10 pt-8 border-t">
            {post.tags.map((tag: string) => (
              <span key={tag} className="px-3 py-1 bg-cream dark:bg-secondary rounded-full text-sm">{tag}</span>
            ))}
          </div>
        )}
      </div>
